...
// ─── DB — Supabase (nuvem, por utilizador) + localStorage (fallback/offline) ──
const DB = {
      ...
      <div className="modal fade-in" style={{ maxWidth: 440, padding: 24 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
          <div style={{ width: 110, height: 158, borderRadius: 10, overflow: "hidden", flexShrink: 0, background: gradientFor(item.id), border: "2px dashed #30363d", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            {loading && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}><span className="spin" style={{ fontSize: 24 }}>◌</span></div>}
            {preview ? <img src={preview} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={() => setPreview("")} /> : <span style={{ color: "#484f58", fontSize: 32 }}>🖼</span>}
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <button onClick={() => fileRef.current?.click()} style={{ padding: "10px", borderRadius: 8, border: "1px dashed #30363d", background: "transparent", color: "#8b949e", cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>📁 Escolher ficheiro</button>
            {item.cover && url !== item.cover && !url.startsWith("data:") && (
              <button onClick={() => { setUrl(item.cover); setPreview(item.cover); }} style={{ padding: "8px", borderRadius: 8, border: "1px solid #30363d", background: "transparent", color: "#8b949e", cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>↩ Restaurar original</button>
            )}
            {url && <button onClick={() => { setUrl(""); setPreview(""); }} style={{ padding: "8px", borderRadius: 8, border: "1px solid #ef444444", background: "transparent", color: "#ef4444", cursor: "pointer", fontFamily: "inherit", fontSize: 12 }}>🗑 Remover capa</button>}
        ...
        <div style={{ padding: "0 24px 24px" }}>
          <div style={{ display: "flex", gap: 16, marginTop: -60, position: "relative", zIndex: 2 }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              ...
              {inLib && <button onClick={() => setCoverEdit(true)} style={{ position: "absolute", bottom: 4, right: 4, width: 26, height: 26, borderRadius: 999, background: accent, border: "none", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }} title="Alterar capa">🖊</button>}
            </div>
          ...
          {item.synopsis && <p style={{ color: "#8b949e", fontSize: 14, lineHeight: 1.7, marginTop: 16 }}>{item.synopsis.slice(0, 500)}{item.synopsis.length > 500 ? "…" : ""}</p>}
    ...
    <div className="card" onClick={() => onOpen(item)} style={{ cursor: "pointer" }}>
      <div style={{ width: "100%", aspectRatio: "2/3", background: gradientFor(item.id), position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: 6, left: 6, right: 6, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          {item.score && <span style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", borderRadius: 6, padding: "2px 6px", fontSize: 11, fontWeight: 700, color: "#fbbf24" }}>★ {item.score}</span>}
        ...
        {libItem?.userRating > 0 && (
          <div style={{ position: "absolute", bottom: 6, left: 6 }}>
            <div style={{ display: "flex", gap: 1 }}>{[1,2,3,4,5].map(s => <span key={s} style={{ fontSize: 10, color: s <= libItem.userRating ? "#f59e0b" : "rgba(255,255,255,0.2)" }}>★</span>)}</div>
          </div>
        )}
    ...
    <div style={{ paddingBottom: 32, maxWidth: 600, margin: "0 auto" }}>
        ...
        <div style={{ height: 160, overflow: "hidden", position: "relative", background: currentBanner ? `url(${currentBanner}) center/cover no-repeat` : `linear-gradient(135deg, ${accent}33 0%, ${bgColor} 100%)` }}>
          ...
          {editing && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              <button onClick={() => bannerRef.current?.click()} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.3)", background: "rgba(0,0,0,0.5)", color: "white", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, backdropFilter: "blur(4px)" }}>🖼 Alterar Banner</button>
              ...
              {bannerUrl && <button onClick={() => { setBannerUrl(""); setBannerPreview(""); }} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #ef444466", background: "rgba(239,68,68,0.15)", color: "#ef4444", cursor: "pointer", fontFamily: "inherit", fontSize: 11 }}>✕ Remover banner</button>}
            </div>
          )}
        </div>
        <div style={{ position: "absolute", bottom: -48, left: "50%", transform: "translateX(-50%)" }}>
          <div style={{ position: "relative", display: "inline-block" }}>
            <div style={{ width: 92, height: 92, borderRadius: 999, overflow: "hidden", background: `linear-gradient(135deg, ${accent}, ${accent}88)`, border: `3px solid ${bgColor}`, boxShadow: `0 0 0 3px ${accent}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {currentAvatar ? <img src={currentAvatar} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 38 }}>👤</span>}
            </div>
            {editing && <>
              <input type="file" accept="image/*" ref={avatarRef} onChange={handleAvatarFile} style={{ display: "none" }} />
              <button onClick={() => avatarRef.current?.click()} style={{ position: "absolute", bottom: 2, right: 2, width: 26, height: 26, borderRadius: 999, background: accent, border: `2px solid ${bgColor}`, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>🖊</button>
            </>}
          </div>
        </div>
      </div>
        ...
        {editing ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 14, flexWrap: "wrap" }}>
              ...
              <button onClick={onSignOut} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #ef444444", background: "#ef444415", color: "#ef4444", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600 }}>⏻ Sair da conta</button>
            </div>
          </>
        )}
      </div>
      ...
      <div style={{ padding: "0 16px" }}>
                    ...
                    <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 12, background: "#161b22", border: "1px solid #21262d", borderRadius: 12, padding: "10px 14px" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ display: "flex", gap: 1 }}>{[1,2,3,4,5].map(s=><span key={s} style={{ fontSize: 12, color: s<=item.userRating?"#f59e0b":"#30363d" }}>★</span>)}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 20 }}>
          {[
            ...
            <div key={s.label} style={{ background: "#0d1117", border: `1px solid ${s.ok?"#10b98133":s.warn?"#eab30833":"#ef444433"}`, borderRadius: 8, padding: "8px 10px" }}>
              <div style={{ fontWeight: 700, fontSize: 12 }}><span style={{ color: s.ok?"#10b981":s.warn?"#eab308":"#ef4444" }}>{s.ok?"✓ ":s.warn?"⚡ ":"! "}</span>{s.icon} {s.label}</div>
              <div style={{ color: "#484f58", fontSize: 11, marginTop: 2 }}>{s.sub}</div>
            </div>
          ))}
        </div>
        {[
          { icon: "☁", title: "Cloudflare Worker — Proxy (IGDB + ComicVine)", val: workerUrl, onSave: onWorkerUrl, placeholder: "https://trackall-proxy.teu-nome.workers.dev", warn: !workerUrl ? "⚡ Sem Worker: jogos limitados a Steam e comics sem resultados." : null, isUrl: true },
          { icon: "🎬", title: "TMDB — Filmes & Séries", val: tmdbKey, onSave: onTmdbKey, placeholder: "Chave TMDB...", warn: !tmdbKey ? "! Sem TMDB, filmes e séries não têm resultados." : null, isUrl: false },
        ...
        <p style={{ fontSize: 11, color: "#484f58", textAlign: "center", marginTop: 8, marginBottom: 20 }}>
          Chaves guardadas na tua conta Supabase — seguras e privadas.
        </p>
      </div>
    </div>
  );
...
export default function TrackAll({ user, onSignOut }) {
  ...
  const updateRating = (id, rating) => { if (!library[id]) return; saveLibrary({ ...library, [id]: { ...library[id], userRating: rating } }); showNotif(rating > 0 ? `${rating} ★` : "Avaliação removida", "#f59e0b"); };
  ...
  const doSearch = useCallback(async (q, type) => {
    ...
    } catch { setSearchError("Erro ao pesquisar. Verifica a tua ligação à internet."); }
      ...
      <div style={{ minHeight: "100vh", background: bgColor, color: "#e6edf3", fontFamily: "'Outfit', 'Segoe UI', sans-serif", paddingBottom: 80 }}>
            ...
            {(!tmdbKey || !workerUrl) && (
              <div onClick={() => setView("profile")} style={{ margin: "12px 16px 0", padding: "12px 16px", borderRadius: 12, cursor: "pointer", background: "linear-gradient(135deg, #1a1f2e, #0d1117)", border: "1px solid #eab30844", display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 11, color: "#8b949e", marginTop: 2 }}>{[!tmdbKey&&"Filmes/Séries",!workerUrl&&"Jogos PS/Xbox/Nintendo + Comics"].filter(Boolean).join(" · ")} precisam de configuração</p>
                </div>
                <span style={{ color: "#eab308", fontSize: 18 }}>→</span>
              </div>
            )}
            <div className="hero-gradient" style={{ padding: "56px 20px 48px", textAlign: "center" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: `rgba(${accentRgb},0.1)`, border: `1px solid rgba(${accentRgb},0.2)`, borderRadius: 999, padding: "5px 14px", marginBottom: 24, fontSize: 12, color: accent, fontWeight: 600 }}>✦ Organiza toda a tua mídia num só lugar</div>
              ...
              <p style={{ color: "#8b949e", fontSize: 16, maxWidth: 480, margin: "0 auto 40px", lineHeight: 1.7 }}>Anime, séries, filmes, manga, livros, manhwa, light novels, jogos e comics.</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, maxWidth: 480, margin: "0 auto 40px" }}>
                {[{l:"Em Curso",v:stats.assistindo,c:accent,e:"▶"},{l:"Completos",v:stats.completo,c:"#10b981",e:"✓"},{l:"Planejados",v:stats.planejado,c:"#06b6d4",e:"⏰"}].map(s => (
        ...
        {view === "search" && (
          <div style={{ padding: "20px 16px" }} className="fade-in">
            ...
            {isSearching && <div style={{ textAlign: "center", padding: "60px 0", color: "#8b949e" }}><div className="spin" style={{ fontSize: 40, display: "block", marginBottom: 12 }}>◌</div><p>A pesquisar...</p></div>}
            {searchError && <div style={{ textAlign: "center", padding: "40px 20px" }}><div style={{ fontSize: 48, marginBottom: 12 }}>😶</div><p style={{ color: "#ef4444", marginBottom: 12 }}>{searchError}</p><p style={{ color: "#484f58", fontSize: 12, lineHeight: 1.6 }}>Se estás a testar no Claude.ai, as chamadas de rede externas são bloqueadas.<br />Funciona no browser normal e no APK.</p></div>}
            {!isSearching && !searchError && searchResults.length === 0 && <div style={{ textAlign: "center", padding: "60px 0", color: "#484f58" }}><div style={{ fontSize: 56, marginBottom: 12 }}>🔍</div><p style={{ marginBottom: 8 }}>Pesquisa algo acima!</p></div>}
        ...
        <nav className="bottom-nav">
          {[{id:"home",icon:"⌂",label:"Início"},{id:"search",icon:"⌕",label:"Pesquisar"},{id:"library",icon:"▤",label:"Biblioteca"},{id:"profile",icon:"◉",label:"Perfil"}].map(n => (
