        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div
            ...
            {(showAllCompleto ? completados : completados.slice(0, 12)).map((item) => {
              const coverSrc = item.customCover || item.cover || item.thumbnailUrl;
              return (
                <div key={item.id} className="recent-card" style={{ flexShrink: 0, width: showAllCompleto ? undefined : 190, cursor: "pointer" }} onClick={() => onOpen && onOpen(item)}>
                  <div style={{ width: showAllCompleto ? "100%" : 190, height: 275, borderRadius: 12, overflow: "hidden", position: "relative", background: gradientFor(item.id), boxShadow: "0 8px 28px rgba(0,0,0,0.6)", transition: "transform 0.18s" }}>
                    {coverSrc
...
function ProfileView({ profile, library, accent, bgColor, bgImage, bgImageMobile, bgSeparateDevices, onBgSeparateDevices, onBgImageMobile, isMobileDevice, bgOverlay, bgBlur, bgParallax, darkMode, statsCardBg, onUpdateProfile, onAccentChange, onBgChange, onBgImage, onBgOverlay, onBgBlur, onBgParallax, onStatsCardBg, onTmdbKey, tmdbKey, workerUrl, onWorkerUrl, onSignOut, userEmail, favorites = [], onToggleFavorite, onImportMihon, onImportPaperback, onImportLetterboxd, driveClientId, onSaveDriveClientId, lastDriveSync, onAutoSync, driveAutoSyncing, onOpen }) {
  ...
  return (
    ...
    <div style={{ paddingBottom: 32, maxWidth: isMobileDevice ? 600 : "100%", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: `1px solid ${darkMode ? "#21262d" : "#e2e8f0"}`, padding: "0 32px", background: darkMode ? bgColor : "#f8fafc", marginBottom: 0 }}>
        {[{ id: "perfil", label: "Perfil" }, { id: "estatisticas", label: "Estatísticas" }].map(t => (
          <button key={t.id} onClick={() => setProfileTab(t.id)} style={{
            ...
            padding: "11px 20px", borderBottom: profileTab === t.id ? `2px solid ${accent}` : "2px solid transparent",
                ...
                {activeTypes.map((t, tIdx) => {
                  ...
                  return (
                    <div key={t.id}>
                      ...
                      {/* Grid 4-col, máx 4 itens por categoria */}
                        ...
                        {favByType[t.id].slice(0, 4).map(item => {
                          ...
                          return (
                            <div key={item.id} className="fav-card-wrap" onClick={() => onOpen && onOpen(item)}
                              ...
                              <div style={{ width: "100%", aspectRatio: "2/3", borderRadius: 9, overflow: "hidden", background: gradientFor(item.id), boxShadow: "0 4px 20px rgba(0,0,0,0.55)", position: "relative" }}>
                                {coverSrc
                                  ? <img src={coverSrc} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => e.currentTarget.style.display = "none"} />
