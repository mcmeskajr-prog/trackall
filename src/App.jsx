            ...
            {inLib ? (
              ...
              <>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {STATUS_OPTIONS.map((s) => (
                    <button key={s.id} onClick={() => { onAdd(item, s.id); onClose(); }} style={{
...
function ProfileView({ profile, library, accent, bgColor, onUpdateProfile, onAccentChange, onBgChange, onTmdbKey, tmdbKey, workerUrl, onWorkerUrl }) {
  ...
  return (
    <div style={{ paddingBottom: 32, maxWidth: 600, margin: "0 auto" }}>
      ...
      {/* ── Deployment & Server ── */}
      <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12, color: "#8b949e" }}>📱 SERVIDOR & APK</h3>

      {[
        {
          icon: "▲", color: "#000000", title: "Vercel (recomendado)",
          badge: "Grátis", badgeColor: "#10b981",
          lines: [
            "Hospeda o teu projeto gratuitamente com HTTPS automático.",
            "Faz upload do ficheiro .jsx → instala Vite → deploy em 2 minutos.",
            "URL permanente tipo: trackall.vercel.app",
          ],
          link: "vercel.com",
        },
        {
          icon: "⚡", color: "#F6821F", title: "Cloudflare Pages",
          badge: "Grátis", badgeColor: "#10b981",
          lines: [
            "Alternativa ao Vercel, igualmente rápido e grátis.",
            "CDN global — carrega rápido em qualquer país.",
          ],
          link: "pages.cloudflare.com",
        },
        {
          icon: "🤖", color: "#3DDC84", title: "APK via Capacitor",
          badge: "Grátis", badgeColor: "#10b981",
          lines: [
            "Converte o projeto web em APK nativo para Android.",
            "Sem CORS — todas as APIs funcionam diretamente.",
            "Requer Node.js + Android Studio (ambos grátis).",
            "Quando estiveres pronto pede-me o guia passo a passo.",
          ],
          link: "capacitorjs.com",
        },
        {
          icon: "☁", color: "#a855f7", title: "Cloudflare Workers (IGDB proxy)",
          badge: "Opcional", badgeColor: "#6e9cf7",
          lines: [
            "Se quiseres usar o TrackAll no browser (não só no APK),",
            "um Worker gratuito faz proxy das chamadas ao IGDB sem expor a chave.",
            "100.000 requests/dia grátis — mais que suficiente para uso pessoal.",
          ],
          link: "workers.cloudflare.com",
        },
      ].map(({ icon, color, title, badge, badgeColor, lines, link }) => (
        <div key={title} style={{ background: "#161b22", border: "1px solid #21262d", borderRadius: 12, padding: 16, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "white", flexShrink: 0 }}>{icon}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 14 }}>{title}</span>
                <span style={{ background: `${badgeColor}22`, color: badgeColor, padding: "1px 7px", borderRadius: 999, fontSize: 10, fontWeight: 700 }}>{badge}</span>
              </div>
              <span style={{ fontSize: 11, color: "#6e9cf7" }}>{link}</span>
            </div>
          </div>
          {lines.map((l, i) => (
            <p key={i} style={{ fontSize: 12, color: "#8b949e", lineHeight: 1.6, paddingLeft: 42 }}>{l}</p>
          ))}
        </div>
      ))}
...
export default function TrackAll() {
  ...
  const addToLibrary = (item, status) => {
    const lib = { ...library, [item.id]: { ...item, userStatus: status, addedAt: Date.now() } };
    saveLibrary(lib);
